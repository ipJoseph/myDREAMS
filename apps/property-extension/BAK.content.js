...
function scrapeSmokyMountainHomes(data) {
  ...
  // Acreage - Look for decimal number followed by "Acres" or "acres"
  const acreageMatch = bodyText.match(/([\d.]+)\s*[Aa]cres?/i) ||
                       bodyText.match(/Parcel\s*Size:\s*([\d.]+)/i);
  if (acreageMatch) {
...
function scrapeZillowByText(data) {
  ...
  // Acreage
  const acreageMatch = bodyText.match(/Size:\s*([\d.]+)\s*Acres?/i);
  if (acreageMatch) data.acreage = parseFloat(acreageMatch[1]);
  
  // Parcel ID
  const parcelMatch = bodyText.match(/Parcel\s*number:\s*(\d+)/i);
  if (parcelMatch) data.parcel_id = parcelMatch[1];
