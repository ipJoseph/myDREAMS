...
function scrapeSmokyMountainHomes(data) {
  ...
  // ==========================================
  // ACREAGE - HIERARCHICAL EXTRACTION (FIXED)
  // ==========================================
  // Priority 1: Structured "Parcel Size:" field
  const parcelSizeMatch = bodyText.match(/Parcel\s*Size:\s*([\d.]+)/i);
  if (parcelSizeMatch) {
    data.acreage = parseFloat(parcelSizeMatch[1]);
    console.log('Acreage from Parcel Size field:', data.acreage);
  }
  
  // Priority 2: Fallback to description text with "acres"
  if (!data.acreage) {
  ...
  // ==========================================
  // PARCEL ID - MULTIPLE PATTERN DETECTION
  // ==========================================
  // Pattern 1: "Parcel ID:" or "Parcel Number:" followed by alphanumeric
  let parcelIdMatch = bodyText.match(/Parcel\s*(?:ID|Number|#):\s*([A-Z0-9\-]+)/i);
  if (parcelIdMatch) {
    data.parcel_id = parcelIdMatch[1].trim();
    console.log('Parcel ID (structured):', data.parcel_id);
  }
  
  // Pattern 2: "Tax ID:" or "Tax Parcel:" (common alternative naming)
  if (!data.parcel_id) {
    parcelIdMatch = bodyText.match(/Tax\s*(?:ID|Parcel):\s*([A-Z0-9\-]+)/i);
    if (parcelIdMatch) {
      data.parcel_id = parcelIdMatch[1].trim();
      console.log('Parcel ID (tax ID):', data.parcel_id);
    }
  }
  
  // Pattern 3: "APN:" (Assessor's Parcel Number - used in some regions)
  if (!data.parcel_id) {
    parcelIdMatch = bodyText.match(/APN:\s*([A-Z0-9\-]+)/i);
    if (parcelIdMatch) {
      data.parcel_id = parcelIdMatch[1].trim();
      console.log('Parcel ID (APN):', data.parcel_id);
    }
  }
