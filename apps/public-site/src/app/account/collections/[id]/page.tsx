...
interface CollectionDetail {
  ...
  derived_from?: { id: string; name: string; slug: string; collection_type: string } | null;
...
export default function CollectionDetailPage() {
  ...
  useEffect(() => {
    ...
    if (!user) return;
    fetchCollection();
  }, [user, authLoading, router, collectionId]);
    ...
    <div className="bg-[var(--color-eggshell)] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {collection.derived_from && (
          <div className="mb-6 px-4 py-3 bg-white border border-gray-200/60 text-sm text-[var(--color-text-light)]">
            Based on:{" "}
            {collection.derived_from.slug ? (
              <Link href={`/collections/${collection.derived_from.slug}`} className="text-[var(--color-accent)] hover:underline">
                {collection.derived_from.name}
              </Link>
            ) : (
              <span className="font-medium">{collection.derived_from.name}</span>
            )}
            {collection.derived_from_type === "template" && " (template)"}
            {collection.derived_from_type === "smart" && " (suggested for you)"}
          </div>
        )}
        ...
        {/* Map view */}
        {viewMode === "map" && collection.listings.length > 0 && (
          <div className="mb-8 border border-gray-200/60 overflow-hidden">
            <CollectionMap listings={collection.listings} height={500} />
          </div>
        )}
