...
# Register blueprint-ified routes. Dependencies are injected via the
# `blueprints.deps` namespace BEFORE importing each blueprint so its
# decorators (e.g. @deps.requires_auth) resolve at module-load time.
...
# Expense report routes + /reports/<path:filename> live in
# apps/property-dashboard/blueprints/expenses.py; registered below
# after module-level db and requires_auth are defined.
    ...
    try:
        with db._get_connection() as conn:
            ...
            ordered = conn.execute(
                'SELECT id FROM package_properties WHERE package_id = ? ORDER BY display_order, added_at',
                [collection_id]
            ).fetchall()
    ...
    with db._get_connection() as conn:
        query = '''
            SELECT id, address, city, county, state, zip, list_price as price,
                   ...
                   elevation_feet, property_type, mls_number, parcel_number,
                   flood_zone, flood_factor, view_potential
            FROM listings
