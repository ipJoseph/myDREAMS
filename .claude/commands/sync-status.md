...
- Use `dreams-db` MCP `run_sql` to count listings by status: `SELECT standard_status, COUNT(*) FROM listings WHERE mls_source='Navica' GROUP BY standard_status`
- Check photo count: `SELECT COUNT(*) FROM listings WHERE mls_source='Navica' AND photo_local_path IS NOT NULL`
...
- Use `dreams-db` MCP `run_sql` to count listings by status: `SELECT standard_status, COUNT(*) FROM listings WHERE mls_source='CanopyMLS' GROUP BY standard_status`
...
- Recent additions (last 24h): `SELECT COUNT(*) FROM listings WHERE created_at > datetime('now', '-1 day')`
- Recent updates (last 24h): `SELECT COUNT(*) FROM listings WHERE updated_at > datetime('now', '-1 day')`
